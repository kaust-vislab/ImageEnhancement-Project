...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.conv1 = conv_2d(7, 7, 3, activation='leakyrelu', padding=1)
        self.conv2 = conv_2d(7, 7, 5, activation='leakyrelu', padding=2)
        self.conv3 = conv_2d(7, 7, 7, activation='leakyrelu', padding=3)
...
class PRIDNet(nn.Module):
    ...
    def forward(self, x):
        ...
        pool1, pool2, pool3,pool4,pool5 = avg_pool(feature_map_2)  # [32, 2, 28, 28] [32, 2, 14, 14] [32, 2, 7, 7]
        # print(pool1.shape,pool2.shape,pool3.shape,pool4.shape,pool5.shape)
        unet1, unet2, unet3,unet4,unet5 = self.all_unet(pool1, pool2, pool3,pool4,pool5)
        # print(unet1.shape,unet2.shape,unet3.shape,unet4.shape,unet5.shape)
        resize1, resize2, resize3,resize4,resize5 = resize_all_image(unet1, unet2, unet3,unet4,unet5)
        fea_cat = torch.cat([feature_map_2, resize1, resize2, resize3,resize4,resize5], 1)
        # print(fea_cat.shape)
